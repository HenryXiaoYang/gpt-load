...
var MessagesEnUS = map[string]string{
	...
	"config.channel_rpm_limit":                  "Channel RPM Limit",
	"config.channel_rpm_limit_desc":             "Maximum requests per minute for this group. 0 disables channel RPM protection.",
	"config.channel_rpm_threshold_percent":      "Channel RPM Protection Threshold (%)",
	"config.channel_rpm_threshold_percent_desc": "After an upstream 429, temporarily reduce the effective RPM limit to this percentage and ramp back up.",
	"config.channel_rpm_ramp_minutes":           "Channel RPM Ramp (minutes)",
	"config.channel_rpm_ramp_minutes_desc":      "Minutes used to ramp the protected RPM limit back to the configured limit after an upstream 429.",
