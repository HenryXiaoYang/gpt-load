...
var MessagesJaJP = map[string]string{
	...
	"config.channel_rpm_limit":                  "チャネル RPM 制限",
	"config.channel_rpm_limit_desc":             "このグループで1分間に許可する最大リクエスト数。0でチャネルRPM保護を無効化します。",
	"config.channel_rpm_threshold_percent":      "チャネル RPM 保護しきい値（%）",
	"config.channel_rpm_threshold_percent_desc": "上流が429を返した後、一時的なRPM上限をこの割合まで下げてから復元します。",
	"config.channel_rpm_ramp_minutes":           "チャネル RPM 復元時間（分）",
	"config.channel_rpm_ramp_minutes_desc":      "上流が429を返した後、保護中のRPM上限を設定値まで戻す時間（分）。",
