...
var MessagesZhCN = map[string]string{
	...
	"config.channel_rpm_limit":                  "渠道 RPM 限制",
	"config.channel_rpm_limit_desc":             "此分组每分钟允许的最大请求数，0 表示关闭渠道 RPM 保护。",
	"config.channel_rpm_threshold_percent":      "渠道 RPM 保护阈值（%）",
	"config.channel_rpm_threshold_percent_desc": "上游返回 429 后，将临时有效 RPM 限制降低到此百分比，再逐步恢复。",
	"config.channel_rpm_ramp_minutes":           "渠道 RPM 恢复时间（分钟）",
	"config.channel_rpm_ramp_minutes_desc":      "上游返回 429 后，将受保护的 RPM 限制恢复到配置值所需的分钟数。",
